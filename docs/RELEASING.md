...
```text
dist/MellowDesk-0.1.0-beta.4-macOS-universal.zip
dist/MellowDesk-0.1.0-beta.4-macOS-universal.zip.sha256
```
...
```bash
ditto -x -k dist/MellowDesk-0.1.0-beta.4-macOS-universal.zip /tmp/mellowdesk-release-check
...
2. Create the immutable GitHub prerelease tag from that SHA, attach the ZIP and SHA-256 file from `dist/`, and use `docs/releases/v0.1.0-beta.4.md` as the release notes. Mark beta versions as prereleases. Never move or recreate a public tag.
