...
小桌伴（MellowDesk）是一款安静待在菜单栏的原生 macOS 桌面伴侣，用一套低打扰的工作间歇计划，轮换提醒你起身活动、日常补水、完成颈肩微运动和提肛跟练。
...
- 默认每 50 分钟提醒一项，按“起身活动 → 起身补水 → 颈肩微运动 → 提肛跟练”轮换；同一时间只出现一张卡片。
- 两分钟起身活动引导、喝水快捷打卡，以及按口渴和个人习惯补水的非定量提示。
- 颈间三动作：左右缓慢转头、左右轻柔侧屈、轻柔低头并回正。
- 两分钟提肛跟练默认开启并进入常规轮换，也可从菜单栏直接开始或在设置中关闭；它不用摄像头、不判断动作，完整练完后会写入本地完成记录和统计。
...
- 小桌伴只在这台 Mac 上保存设置、训练汇总和喝水/起身/提肛完成时间，不保存图像、人脸模板、音频或逐帧头部角度。
...
请从 [v0.1.0-beta.7 Release](https://github.com/EigenLogic/MellowDesk/releases/tag/v0.1.0-beta.7)
...
2. 默认计划会在工作时段内轮换提醒起身、补水、颈肩活动和提肛跟练；也可从菜单栏手动开始任一项。
3. 两分钟提肛跟练默认开启，也可在设置中关闭；完整练完后会增加一条本地完成记录。
...
当前仓库包含 **85 个确定性测试**。合成数据自动测试不能替代真实摄像头验收。
...
- [安全政策](SECURITY.md)
- [变更记录](CHANGELOG.md)
- [v0.1.0-beta.7 发布说明](docs/releases/v0.1.0-beta.7.md)
- [v0.1.0-beta.6 发布说明](docs/releases/v0.1.0-beta.6.md)
